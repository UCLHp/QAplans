...
def spot_parameters(plan_param=None):
    ...
    #  if not a csv input then need to generate the dataset
        ...
        #     gAngle, Emin, Emax, delE, Nx, Ny, Sep, sMU = ([float(gAngle)], float(Emin), float(Emax), float(delE), int(Nx), int(Ny), float(Sep), float(sMU))
            ...
            planName, gAngle, E, Nx, Ny, Sep, sMU = multenterbox(title=bxTitle, msg=bxMsg, fields=bxOpts, values=bxVals)
            gAngle, E, Nx, Ny, Sep, sMU = ([float(gAngle)], list(float(_) for _ in E.split(',')), int(Nx), int(Ny), float(Sep), float(sMU))
        ...
        for an in gAngle:
            for en in E: #numpy.arange(Emin, Emax+delE, delE):
                for x in range(Nx):
                    for y in range(Ny):
                        data.append( [an, en, \
                                      (x-((Nx-1)/2))*Sep, \
